package com.sudoplay.mc.watercan.modules.watercan.client.event;
...
public class RenderSpecificHandEventHandler {
  ...
  private static boolean isItemWatercan(Item item) {
    ...
    return item == ModuleItems.WATERCAN_WOOD
        || item == ModuleItems.WATERCAN_STONE
        || item == ModuleItems.WATERCAN_IRON
        || item == ModuleItems.WATERCAN_DIAMOND
        || item == ModuleItems.WATERCAN_GOLD;
  }
  ...
  private static void renderItemInFirstPerson(
    ...
    RayTraceResult rayTraceResult = rayTrace(clientPlayer, 10.0, partialTicks, true);
    ...
    transformSideFirstPerson(enumhandside, equipProgress * equipProgressScalar);
    transformFirstPerson(enumhandside);
    renderItemSide(
        clientPlayer,
        itemStack,
  ...
  private static void transformFirstPerson(EnumHandSide handSide) {
